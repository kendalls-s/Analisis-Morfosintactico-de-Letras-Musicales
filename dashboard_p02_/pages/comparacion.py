...
layout = html.Div(className="page-inner", children=[
    html.Div(className="section-intro", children=[
        html.Div(children=[
            ...
            html.P("Evaluación con Regresión Logística y KNN (k=5) sobre partición 80/20 "
                   "estratificada. K-Means + Silhouette Score para evaluar separabilidad "
                   "de géneros sin usar etiquetas."),
        ]),
    ]),
    ...
    card("Silhouette Score — K-Means Clustering",
         "k = número de géneros · métrica coseno", "fig-comp-sil"),

    card("F1-Score por Representación y Género",
         "Logistic Regression · verde = mejor rendimiento",
         "fig-comp-f1", tall=True, full=True),

    html.Div(className="grid-2", children=[
        card("Matriz de Confusión — BERT (mejor modelo)",
             "Partición 80/20 estratificada · Logistic Regression",
             "fig-comp-confusion", tall=True),
...
def render_kpis(_):
    ...
    best_lr   = df["lr"].max()  if not df.empty else 0
    best_sil  = sil["score"].max() if not sil.empty else 0
    tfidf_lr  = df[df["rep"]=="TF-IDF"]["lr"].values[0]  if not df.empty else 0
    mejora    = best_lr - tfidf_lr
    return [
        kpi(f"{tfidf_lr*100:.1f}%", "TF-IDF Accuracy",      "Logistic Regression"),
        kpi(f"{df[df['rep']=='Word2Vec']['lr'].values[0]*100:.1f}%" if not df.empty else "—",
            "Word2Vec Accuracy", "Logistic Regression"),
        kpi(f"{best_lr*100:.1f}%",  "BERT Accuracy",         "Mejor resultado", accent=True),
        kpi(f"+{mejora*100:.1f} pp","Mejora BERT vs TF-IDF", "Ganancia por representación contextual"),
    ]
...
def fig_sil(_):
    ...
    fig.update_layout(**base_layout(
        margin=dict(t=30,r=20,b=40,l=55),
        yaxis=dict(title=dict(text="Silhouette Score (coseno)",
...
def fig_f1(_):
    ...
    fig.update_layout(**base_layout(margin=dict(t=10,r=70,b=60,l=90)))
    return fig
...
def fig_confusion(_):
    ...
    fig.update_layout(**base_layout(
        margin=dict(t=10,r=20,b=70,l=90),
...
def fig_sparsity(_):
    from data import _load_embedding_data
    df  = _load_embedding_data()
    n_w2v  = len(df["w2v"].iloc[0])  if not df.empty and len(df) > 0 else 100
    n_bert = len(df["bert"].iloc[0]) if not df.empty and len(df) > 0 else 768
    dims   = [5000, n_w2v, n_bert]
    spars  = [96.8, 0.0, 0.0]
    ...
    fig = go.Figure(go.Scatter(
        ...
        textposition="top center", textfont=dict(color=PALETTE["text"],size=12),
        marker=dict(color=colors,size=sizes,opacity=.88),
        hovertemplate="<b>%{text}</b><br>Dimensiones: %{x}<br>Dispersión: %{y:.1f}%<extra></extra>",
    ))
    fig.update_layout(**base_layout(
        margin=dict(t=30,r=40,b=50,l=65),
        xaxis=dict(type="log",title=dict(text="Dimensionalidad (log)",
                                         font=dict(size=10,color=PALETTE["muted"])),
                   gridcolor=PALETTE["border"],zeroline=False,tickfont=dict(color=PALETTE["muted"])),
        yaxis=dict(range=[-5,108],title=dict(text="Dispersión (%)",
                                              font=dict(size=10,color=PALETTE["muted"])),
                   gridcolor=PALETTE["border"],zeroline=False,tickfont=dict(color=PALETTE["muted"])),
    ))
    return fig
    ...
    for col_idx, rep in enumerate(reps, 1):
        ...
        for g in GENRES:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(color=GENRE_COLORS.get(g,PALETTE["accent"]),size=5,opacity=.65,line=dict(width=0)),
                hovertemplate=f"<b>{g}</b><extra></extra>",
            ), row=1, col=col_idx)
    fig.update_layout(**base_layout(
        legend=dict(orientation="h",y=-0.12,font=dict(color=PALETTE["muted"],size=10),bgcolor="rgba(0,0,0,0)"),
