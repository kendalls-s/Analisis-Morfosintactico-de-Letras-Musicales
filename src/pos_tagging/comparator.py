def comparar_nltk_spacy_csv(df_nltk, df_spacy, indice=0):
    ...
    for token in tokens_comunes:
        ...
        print(f"{token:<20} {tag_nltk:<15} {upos:<18} {fine:<15} {lemma}{marca}")
    ...
    print(f"Acuerdo           : {acuerdos}/{total} ({acuerdos/total*100:.1f}%)")
    print(f"Tokens solo NLTK  : {len(nltk_dict)  - total} (editadas)")
    print(f"Tokens solo spaCy : {len(spacy_dict) - total} (editadas)")
